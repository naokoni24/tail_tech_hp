...
- ヒーロー右側のサービス利用イメージは、スマホとPCの緩やかな浮遊、情報線・グラフ・説明バッジの軽い動きをCSSでループ表示する。スマホ画面内では「写真を選択 → イベントを選択 → 生成ボタンを押す」の3ステップを順番にハイライトした後、生成後の夏祭りフォトが端末の左上へ飛び出すカード演出を表示する。カードには「できました！」ラベルときらめきを付ける。`prefers-reduced-motion`では飛び出した位置に生成後の写真を静止表示する。
...
- 4つの悩みカードを表示し、該当するサービスカードへアンカーリンクする。
  ...
  4. Excel・紙での管理に限界を感じている → システム提案・開発
